...
# Desired gain R from 0 to 1, step size u
# Параметры автоматической регулировки усиления
# Желаемое усиление R от 0 до 1, размер шага u
agc_desired_gain = 1
agc_step_size = 0.05

# Gain error value and gain value
# Значение ошибки усиления и значение усиления
gain_error = 0
gain_value = agc_desired_gain
...
for i in range(int(input_signal_length)):
    ...
    average_buffer[average_buffer_counter] = abs(output_signal_data[i])
    average_value = np.mean(average_buffer)

    # Calculate gain error and gain value
    # Вычисление ошибки усиления и значения усиления
    gain_error = agc_desired_gain - average_value
    gain_error *= agc_step_size
    gain_value = gain_value + gain_error
